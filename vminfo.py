...
Usage: python vminfo.py -s 10.1.1.230 -u whpark@vsphere.local -p VMWare6!123 -m kerpdb -c  -i 300
"""
...
def PrintVmInfo(vm, content, vchtime, interval, perf_dict, ):
    ...
    try:
        ...
        print('Drive Info                     :', diskinfo_print_list[0])
        if len(diskinfo_print_list) > 1:
            diskinfo_print_list.pop(0)
            for each_disk in diskinfo_print_list:
                print('                                ', each_disk)
